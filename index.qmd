...
complexity.reg <- complexity.df |> 
  ...
  filter(!is.na(log_bus_entries),
         !is.na(business_high_turnover)) |> 
  st_as_sf() 



fit.ols <- lm(city_complexity ~ is_cc + log_employment + log_population + log_hh_income + log_bus_entries + business_share + business_high_turnover,
              data = complexity.reg)
...
Regression output is summarised in @tbl-regression-results.
