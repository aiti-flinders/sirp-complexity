...
complexity.reg <- complexity.df |> 
  ...
  ungroup() |> 
  mutate(is_cc = fct(ifelse(str_detect(gcc_name_2021, "Greater"), "1", "0")),
         log_employment = log(total_employment),
         log_population = log(population),
         log_hh_income = log(median_hh_inc),
         log_bus_entries = log(total_business_entries)) |> 
  filter(!is.na(log_bus_entries))



fit.ols <- lm(city_complexity ~ is_cc + log_employment + log_population + log_hh_income + log_bus_entries + business_share,
              data = complexity.reg)
...
fit.lag <- lagsarlm(city_complexity ~ is_cc + log_employment + log_population + log_hh_income + log_bus_entries + business_share,
                     data = complexity.reg,
                    listw = lw, zero.policy = TRUE)

fit.err <- errorsarlm(city_complexity ~ is_cc + log_employment + log_population + log_hh_income + log_bus_entries + business_share,
                     data = complexity.reg,
                    listw = lw, zero.policy = TRUE)
...
#| tbl-cap: Regression summary
huxreg(fit.ols, fit.lag, fit.err)
