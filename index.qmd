...
-   Dataset covers `r length(unique(pow_clean$sa3))` regions and `r length(unique(pow_clean$industry_occupation))` industry-occupations. @fig-employment-density shows the presence of any level of employment within a region and industry-occupation. As can be seen, there is a high level of employment density across our data.
